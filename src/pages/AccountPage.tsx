...
// ── Static mock data (replace with MongoDB queries later) ──────────────────────
const MOCK_ORDERS = [
      ...
      <div className="max-w-md mx-auto px-4 py-12">
        <div className="flex flex-col items-center mb-8 gap-2">
          <div className="w-12 h-12 rounded-full bg-[#F5EDE6] flex items-center justify-center">
            <User size={24} className="text-[#B31217]" />
          </div>
        ...
        <div className="flex border border-border mb-6">
          <button className={`flex-1 py-2.5 text-sm font-medium tracking-wider transition-colors ${mode === "login" ? "bg-[#B31217] text-white" : "hover:bg-[#F5EDE6]"}`} onClick={() => { setMode("login"); setError(""); }}>LOGIN</button>
          <button className={`flex-1 py-2.5 text-sm font-medium tracking-wider transition-colors ${mode === "register" ? "bg-[#B31217] text-white" : "hover:bg-[#F5EDE6]"}`} onClick={() => { setMode("register"); setError(""); }}>REGISTER</button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {mode === "register" && (
            <div>
              ...
              <input type="text" required value={form.name} onChange={e => set("name", e.target.value)} placeholder="Your name" className="w-full border border-border bg-background px-3 py-2.5 text-sm focus:outline-none focus:border-[#B31217] transition-colors" />
            </div>
          )}
          <div>
            <label className="block text-xs font-medium tracking-wider text-muted-foreground mb-1.5">{mode === "login" ? "USERNAME" : "EMAIL"}</label>
            <input type={mode === "login" ? "text" : "email"} required value={mode === "login" ? form.username : form.email} onChange={e => set(mode === "login" ? "username" : "email", e.target.value)} placeholder={mode === "login" ? "username" : "you@example.com"} className="w-full border border-border bg-background px-3 py-2.5 text-sm focus:outline-none focus:border-[#B31217] transition-colors" />
          </div>
          <div>
            <label className="block text-xs font-medium tracking-wider text-muted-foreground mb-1.5">PASSWORD</label>
            <input type="password" required value={form.password} onChange={e => set("password", e.target.value)} placeholder="••••••••" className="w-full border border-border bg-background px-3 py-2.5 text-sm focus:outline-none focus:border-[#B31217] transition-colors" />
          </div>
          {mode === "register" && (
            <div>
              <label className="block text-xs font-medium tracking-wider text-muted-foreground mb-1.5">CONFIRM PASSWORD</label>
              <input type="password" required value={form.confirm} onChange={e => set("confirm", e.target.value)} placeholder="••••••••" className="w-full border border-border bg-background px-3 py-2.5 text-sm focus:outline-none focus:border-[#B31217] transition-colors" />
            </div>
          )}
          {error && <p className="text-sm text-destructive">{error}</p>}
          <button type="submit" className="w-full py-3 bg-[#B31217] text-white text-sm font-medium tracking-wider hover:opacity-90 transition-opacity mt-2">
            {mode === "login" ? "SIGN IN" : "CREATE ACCOUNT"}
          </button>
        </form>
      </div>
    ...
    <div className="min-h-screen bg-background pb-16 md:pb-0">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 flex flex-col md:flex-row gap-6">
        ...
        <aside className="md:w-60 shrink-0">
          <div className="bg-[#F5EDE6] rounded-sm p-4 mb-4 flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-[#B31217] flex items-center justify-center text-white font-bold text-sm">
              {TEST_USER.name[0]}
            </div>
            <div>
          ...
          <nav className="space-y-0.5">
            {menuItems.map(({ id, icon: Icon, label }) => (
              <button key={id} onClick={() => setSection(id)}
                className={`w-full flex items-center justify-between px-3 py-2.5 text-sm rounded-sm transition-colors ${section === id ? "bg-[#B31217] text-white" : "hover:bg-[#F5EDE6] text-foreground"}`}>
                <span className="flex items-center gap-2.5"><Icon size={15} />{label}</span>
                <ChevronRight size={14} />
              </button>
            ))}
            <button onClick={() => { setLoggedIn(false); navigate("/"); }}
              className="w-full flex items-center gap-2.5 px-3 py-2.5 text-sm text-destructive hover:bg-red-50 rounded-sm transition-colors mt-2">
        ...
        <main className="flex-1 min-w-0">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  ...
                  <button key={item.label} onClick={() => setSection(item.id)}
                    className="bg-[#F5EDE6] rounded-sm p-4 text-center hover:bg-[#B31217] hover:text-white transition-colors group">
              ...
              {MOCK_ORDERS.map(order => (
                <div key={order.id} className="border border-border rounded-sm p-4 space-y-2">
                  ...
                  <div className="flex gap-2 pt-1">
                    <button onClick={() => { setTrackInput(order.id); setSection("track"); }} className="text-xs text-[#B31217] underline">Track</button>
                    <button className="text-xs text-[#B31217] underline">Invoice</button>
                    {order.status === "Delivered" && <button onClick={() => setSection("service")} className="text-xs text-[#B31217] underline">Return / Refund</button>}
                  </div>
                </div>
              ))}
            </div>
          ...
          {section === "track" && (
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                <input value={trackInput} onChange={e => setTrackInput(e.target.value)} placeholder="Enter Order ID or Tracking Number"
                  className="flex-1 border border-border px-3 py-2.5 text-sm focus:outline-none focus:border-[#B31217] transition-colors" />
                <button onClick={handleTrack} className="px-5 py-2.5 bg-[#B31217] text-white text-sm font-medium hover:opacity-90 transition-opacity">TRACK</button>
              </div>
              {error && <p className="text-sm text-destructive">{error}</p>}
              {trackedOrder && (
                  ...
                  <div className="space-y-3 pt-2">
                    {[
                      ...
                      <div key={i} className="flex items-center gap-3">
                        <div className={`w-3 h-3 rounded-full shrink-0 ${step.done ? "bg-[#B31217]" : "bg-gray-200"}`} />
                ...
                <div key={item.id} className="border border-border rounded-sm p-4 flex items-center justify-between gap-4">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-sm font-bold text-[#B31217]">₹{item.price.toLocaleString()}</span>
                      <span className="text-xs text-muted-foreground line-through">₹{item.originalPrice.toLocaleString()}</span>
                    </div>
                  </div>
                  <button onClick={() => navigate("/cart")} className="shrink-0 px-4 py-2 bg-[#B31217] text-white text-xs font-medium hover:opacity-90 transition-opacity">ADD TO CART</button>
                </div>
              ))}
            </div>
          )}
            ...
            <div className="space-y-4">
              ...
              {MOCK_ORDERS.filter(o => o.status === "Delivered").map(order => (
                <div key={order.id} className="border border-border rounded-sm p-4 flex items-center justify-between gap-4">
                  ...
                  <button onClick={() => navigate("/cart")} className="shrink-0 px-4 py-2 bg-[#B31217] text-white text-xs font-medium hover:opacity-90 transition-opacity">BUY AGAIN</button>
                </div>
              ))}
            </div>
          )}
              ...
              {MOCK_ADDRESSES.map(addr => (
                <div key={addr.id} className="border border-border rounded-sm p-4 space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold tracking-wider bg-[#F5EDE6] text-[#B31217] px-2 py-0.5 rounded-full">{addr.label}</span>
                  ...
                  <div className="flex gap-3 pt-1">
                    <button className="text-xs text-[#B31217] underline">Edit</button>
                    <button className="text-xs text-[#B31217] underline">Remove</button>
                  </div>
                </div>
              ))}
              <button className="w-full py-2.5 border border-[#B31217] text-[#B31217] text-sm font-medium tracking-wider hover:bg-[#F5EDE6] transition-colors">+ ADD NEW ADDRESS</button>
            </div>
          )}
          ...
          {section === "rewards" && (
            <div className="space-y-4">
              ...
              <div className="bg-[#B31217] text-white rounded-sm p-6 space-y-2">
                ...
                <div className="w-full bg-white/20 rounded-full h-1.5 mt-2">
                  <div className="bg-white h-1.5 rounded-full" style={{ width: `${(MOCK_REWARDS.points / (MOCK_REWARDS.points + MOCK_REWARDS.pointsToNext)) * 100}%` }} />
                </div>
              </div>
              <div className="border border-border rounded-sm p-4 space-y-2">
                ...
                {[["Every ₹100 spent", "10 pts"], ["Write a review", "25 pts"], ["Refer a friend", "100 pts"], ["Birthday bonus", "50 pts"]].map(([action, pts]) => (
                  <div key={action} className="flex justify-between text-sm py-1 border-b border-border last:border-0">
                    <span>{action}</span><span className="font-medium text-[#B31217]">{pts}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {[
                  ...
                  <button key={label} className="border border-border rounded-sm p-4 text-left hover:border-[#B31217] hover:bg-[#F5EDE6] transition-colors space-y-2">
                    <Icon size={20} className="text-[#B31217]" />
              ...
              <div className="border border-border rounded-sm p-4 space-y-3">
                ...
                <select className="w-full border border-border px-3 py-2.5 text-sm focus:outline-none focus:border-[#B31217] bg-background">
                ...
                <textarea rows={3} placeholder="Describe your issue..." className="w-full border border-border px-3 py-2.5 text-sm focus:outline-none focus:border-[#B31217] resize-none" />
                <button className="px-6 py-2.5 bg-[#B31217] text-white text-sm font-medium hover:opacity-90 transition-opacity">SUBMIT TICKET</button>
              </div>
            </div>
          )}
